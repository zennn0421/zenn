...
const USER_PASSWORD = process.env.USER_PASSWORD || "0421"; // Fallback for local testing
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || "Zen0421"; // Fallback for local testing

let users = {}; // { socket.id: { username, role } }
let messages = []; // { id, username, content, type, timestamp }

// 静的ファイルの提供
// app.use(express.static(path.join(__dirname, 'public'))); // Vercel handles static files
