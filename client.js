...
function setUsername() {
    ...
    if (name) {
        username = name;
        socket.emit('userJoin', { username, isAdmin: userRole === 'admin' });
...
form.addEventListener('submit', (e) => {
  ...
  if (input.value && username) {
    console.log('Emitting message:', { content: input.value, type: 'text', username: username });
    socket.emit('message', { content: input.value, type: 'text', username: username });
    input.value = '';
  }
});
...
imageInput.addEventListener('change', (e) => {
    ...
    if (file) {
        ...
        reader.onload = (event) => {
            console.log('Emitting image message:', { content: event.target.result, type: 'image', username: username });
            socket.emit('message', { content: event.target.result, type: 'image', username: username });
        };
